            ...
            <div class="session-card__body">
               ...
               <div class="session-card__signers">
                  <div v-for="(signer, index) in session.signers" :key="index" class="session-card__signer">
                     <FontAwesomeIcon
                        :icon="signed(signer) ? 'check' : 'times'"
                        class="session-card__icon"
                        :class="{ valid: signed(signer) }"
                     />
...
import {
   SignSessionInWork,
   SignSessionSignerStatuses,
   SignSessionStatuses,
   SignSessionViews
} from '@/types/sign-sessions';

const model = computed<View<SignSessionInWork>>(() => {
   return store.getters['navigation/getView'](SignSessionViews.InWork);
});

const signed = (signer) => signer.signStatusId === SignSessionSignerStatuses.Signed;

const { changePage } = useViewPagination(SignSessionViews.InWork, model);
const { chunkedSessionList } = useChunkedSignSessionList<SignSessionInWork>(model);
</script>
